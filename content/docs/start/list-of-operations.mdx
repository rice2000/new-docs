...
Creates a ClaimableBalanceEntry. (TODO: Link to Claimable Balance glossary)
...
| Claimants | list of claimants | List of Claimants (account address and ClaimPredicate pair) (TODO: Link to Claimable Balance glossary) that can claim this ClaimableBalanceEntry. |

Possible errors:

| Error | Code | Description |
| --- | --- | --- |
| CREATE_CLAIMABLE_BALANCE_MALFORMED | -1 | The input to this operation is invalid. See glossary for more details (TODO: Link to Claimable Balance glossary ) |
| CREATE_CLAIMABLE_BALANCE_LOW_RESERVE | -2 | The account creating this entry does not have enough XLM to satisfy the minimum XLM reserve increase caused by adding a ClaimableBalanceEntry. For every claimant in the list, the minimum amount of XLM this account must hold will increase. |
...
| Parameters | Type | Description |
| --- | --- | --- |
| BalanceID | claimableBalanceID TODO: Link to Claimable Balance glossary | balanceID on the ClaimableBalanceEntry that the source account is claiming. |
...
Establishes the is-sponsoring-future-reserves-for relationship between the source account and sponsoredID. See link on how this operation should be used. TODO: Add link to sponsorship glossary page.

There must be a corresponding [end sponsoring future reserves](#end-sponsoring-future-reserves) operation in the same transaction to end the is-sponsoring-future-reserves-for relationship. The transaction will fail otherwise.
...
If the source account is not sponsored or is sponsored by the owner of the specified entry or sub-entry, then attempt to revoke the sponsorship. If the source account is sponsored, then attempt to transfer the sponsorship to the sponsor of the source account.
...
1. REVOKE_SPONSORSHIP_LEDGER_ENTRY

| Parameters | Type | Description |
| --- | --- | --- |
| LedgerKey | ledgerKey | Ledger key that corresponds to the ledgerEntry that may have it's sponsorship modified. |

2. REVOKE_SPONSORSHIP_SIGNER

| Parameters | Type | Description |
| --- | --- | --- |
| Signer | {account ID, Signer Key} | Signer that may have it's sponsorship modified. |
...
| REVOKE_SPONSORSHIP_LOW_RESERVE | -3 | The source account does not have enough XLM to satisfy the minimum reserve caused by revoking sponsorship on a ledgerEntry/signer it owns, or the sponsor of the source account doesn't have enough XLM to satisfy the minimum reserve caused by sponsoring a new ledgerEntry/signer.  |
| REVOKE_SPONSORSHIP_ONLY_TRANSFERABLE | -4 | Sponsorship cannot be removed from this ledgerEntry |